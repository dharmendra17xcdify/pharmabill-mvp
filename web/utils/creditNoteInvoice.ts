...
export function buildCreditNoteHTML(
  ret: SupplierReturn & { items: SupplierReturnItem[] },
  ...
  const itemRows = ret.items
    ...
    <tr>
      ...
      <td style="text-align:center">${item.return_reason || ''}</td>
    </tr>`
    )
    .join('');
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .header h2 { font-size: 14px; color: #D32F2F; letter-spacing: 0.5px; font-weight: bold; margin-top: 4px; }
...
<body>
  <div class="header">
    ...
    ${gstinLine}
    ${licLine}
    <h2>CREDIT NOTE / RETURN TO SUPPLIER</h2>
  </div>
  <hr class="divider"/>
  <div class="bill-meta">
    <div>
      <strong>Return No:</strong> ${ret.return_number}<br/>
      <strong>Date:</strong> ${formatDate(ret.created_at)}<br/>
      ${ret.credit_note_no ? `<strong>Credit Note No:</strong> ${ret.credit_note_no}<br/>` : ''}
    </div>
    <div style="text-align:right">
      <strong>Supplier:</strong> ${ret.supplier_name || '—'}<br/>
      <strong>Original Invoice No:</strong> ${ret.supplier_invoice_no || '—'}<br/>
      ${ret.supplier_gstin ? `<strong>GSTIN:</strong> ${ret.supplier_gstin}<br/>` : ''}
      ${ret.supplier_phone ? `<strong>Tel:</strong> ${ret.supplier_phone}<br/>` : ''}
      ${ret.supplier_drug_license ? `<strong>Drug License:</strong> ${ret.supplier_drug_license}<br/>` : ''}
      ${ret.supplier_address ? `<span style="font-size:10px;color:#555">${ret.supplier_address}</span>` : ''}
    </div>
  </div>
  <hr class="divider"/>
  <table>
    <thead>
      <tr>
        ...
        <th style="min-width:70px">Return Reason</th>
      </tr>
    </thead>
    <tbody>${itemRows}</tbody>
    <tfoot>
      <tr style="background:#EEF2FF;font-weight:bold">
        <td colspan="5" style="text-align:right;padding:4px 8px">
          Total Items: ${ret.items.length}
        </td>
        <td style="text-align:center">${totalQty}</td>
        <td colspan="4"></td>
        <td style="text-align:right">₹${Number(ret.grand_total).toFixed(2)}</td>
        <td colspan="3"></td>
      </tr>
    </tfoot>
  </table>
  <div class="totals-section">
    <table class="totals-table">
      <tr><td>Subtotal (Taxable)</td><td style="text-align:right">₹${Number(ret.subtotal).toFixed(2)}</td></tr>
      <tr><td>CGST Total</td><td style="text-align:right">₹${Number(ret.cgst_total).toFixed(2)}</td></tr>
      <tr><td>SGST Total</td><td style="text-align:right">₹${Number(ret.sgst_total).toFixed(2)}</td></tr>
      <tr class="grand"><td><strong>Grand Total</strong></td><td style="text-align:right"><strong>₹${Number(ret.grand_total).toFixed(2)}</strong></td></tr>
    </table>
  </div>
  ...
  <div class="footer">
    <p>This is a computer-generated Credit Note / Return to Supplier document.</p>
  </div>
</body>
</html>`;
}
