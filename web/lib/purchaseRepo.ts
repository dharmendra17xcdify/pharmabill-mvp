...
export async function savePurchase(
  ...
  try {
    const purchaseResult = await new sql.Request(transaction)
      ...
      .input('notes', sql.NVarChar(1000), purchase.notes || '')
      .input('created_at', sql.NVarChar(50), purchase.created_at)
      .query(`
        INSERT INTO purchases
          (purchase_number, supplier_name, supplier_invoice_no, supplier_gstin, supplier_address, supplier_phone, supplier_drug_license, subtotal, cgst_total, sgst_total, discount_total, grand_total, payment_mode, notes, created_at)
        OUTPUT INSERTED.id
        VALUES
          (@purchase_number, @supplier_name, @supplier_invoice_no, @supplier_gstin, @supplier_address, @supplier_phone, @supplier_drug_license, @subtotal, @cgst_total, @sgst_total, @discount_total, @grand_total, @payment_mode, @notes, @created_at)
      `);
...
export async function updatePurchase(
  id: number,
  purchase: Omit<Purchase, 'id' | 'purchase_number'>,
  items: Omit<PurchaseItem, 'id' | 'purchase_id'>[]
): Promise<void> {
  const pool = await getPool();
  const transaction = new sql.Transaction(pool);
  ...
  try {
    ...
    const oldItems = await new sql.Request(transaction)
      .input('pid_r', sql.Int, id)
      .query(`SELECT medicine_id, qty, deal_qty, batch_no FROM purchase_items WHERE purchase_id = @pid_r`);

    for (const old of oldItems.recordset) {
      const receivedQty = (old.qty || 0) + (old.deal_qty || 0);
      if (old.medicine_id && receivedQty > 0) {
        const batchKey = (old.batch_no || '').trim();
        if (batchKey) {
          await new sql.Request(transaction)
            .input('r_mid', sql.Int, old.medicine_id)
            .input('r_bn',  sql.NVarChar(100), batchKey)
            .input('r_qty', sql.Int, receivedQty)
            .query(`
              UPDATE medicine_batches
              SET stock_qty = CASE WHEN stock_qty - @r_qty < 0 THEN 0 ELSE stock_qty - @r_qty END
              WHERE medicine_id = @r_mid AND batch_no = @r_bn
            `);
        } else {
          await new sql.Request(transaction)
            .input('r_mid2', sql.Int, old.medicine_id)
            .input('r_qty2', sql.Int, receivedQty)
            .query(`
              UPDATE medicine_batches
              SET stock_qty = CASE WHEN stock_qty - @r_qty2 < 0 THEN 0 ELSE stock_qty - @r_qty2 END
              WHERE id = (
                SELECT TOP 1 id FROM medicine_batches WHERE medicine_id = @r_mid2 ORDER BY id DESC
              )
            `);
        }
      }
    ...
    // Delete old items
    await new sql.Request(transaction)
      .input('pid_d', sql.Int, id)
      .query(`DELETE FROM purchase_items WHERE purchase_id = @pid_d`);
    ...
    await new sql.Request(transaction)
      ...
      .query(`
        UPDATE purchases SET
          supplier_name = @supplier_name, supplier_invoice_no = @supplier_invoice_no,
          supplier_gstin = @supplier_gstin, supplier_address = @supplier_address,
          supplier_phone = @supplier_phone, supplier_drug_license = @supplier_drug_license,
          subtotal = @subtotal, cgst_total = @cgst_total, sgst_total = @sgst_total,
          discount_total = @discount_total, grand_total = @grand_total,
          payment_mode = @payment_mode, notes = @notes, created_at = @created_at
        WHERE id = @id
      `);

    // Insert new items and update batch stock
    for (let i = 0; i < items.length; i++) {
      const item = items[i];
      await new sql.Request(transaction)
        .input(`purchase_id_${i}`,    sql.Int,           id)
        .input(`medicine_id_${i}`,    sql.Int,           item.medicine_id ?? null)
        .input(`medicine_name_${i}`,  sql.NVarChar(255), item.medicine_name || '')
        .input(`hsn_${i}`,            sql.NVarChar(50),  item.hsn || '')
        .input(`batch_no_${i}`,       sql.NVarChar(100), item.batch_no || '')
        ...
        .query(`
          INSERT INTO purchase_items
            (purchase_id, medicine_id, medicine_name, hsn, batch_no, expiry_month, expiry_year,
             packing, pack_qty, qty, deal_qty, rate, discount, cgst_percent, sgst_percent,
             cgst_amount, sgst_amount, taxable_amount, amount, mrp, manufacture_name)
          VALUES
            (@purchase_id_${i}, @medicine_id_${i}, @medicine_name_${i}, @hsn_${i}, @batch_no_${i},
             @expiry_month_${i}, @expiry_year_${i}, @packing_${i}, @pack_qty_${i}, @qty_${i},
             @deal_qty_${i}, @rate_${i}, @discount_${i}, @cgst_percent_${i}, @sgst_percent_${i},
             @cgst_amount_${i}, @sgst_amount_${i}, @taxable_amount_${i}, @amount_${i},
             @mrp_${i}, @manufacture_name_${i})
        `);

      // Re-apply batch stock
      const receivedQty = item.qty + (item.deal_qty || 0);
      if (item.medicine_id && receivedQty > 0) {
        const batchKey = (item.batch_no || '').trim();
        if (batchKey) {
          await new sql.Request(transaction)
            .input(`upsert_mid_${i}`,  sql.Int,           item.medicine_id)
            .input(`upsert_bn_${i}`,   sql.NVarChar(100), batchKey)
            .input(`upsert_qty_${i}`,  sql.Int,           receivedQty)
            .input(`upsert_mrp_${i}`,  sql.Decimal(10,2), item.mrp || 0)
            .input(`upsert_rate_${i}`, sql.Decimal(10,2), item.rate || 0)
            .query(`
              IF EXISTS (SELECT 1 FROM medicine_batches WHERE medicine_id = @upsert_mid_${i} AND batch_no = @upsert_bn_${i})
                UPDATE medicine_batches SET stock_qty = stock_qty + @upsert_qty_${i},
                  mrp = @upsert_mrp_${i}, rate = @upsert_rate_${i}
                WHERE medicine_id = @upsert_mid_${i} AND batch_no = @upsert_bn_${i}
              ELSE
                INSERT INTO medicine_batches (medicine_id, batch_no, mrp, selling_price, rate, stock_qty, created_at)
                VALUES (@upsert_mid_${i}, @upsert_bn_${i}, @upsert_mrp_${i}, @upsert_mrp_${i}, @upsert_rate_${i}, @upsert_qty_${i}, '')
            `);
        } else {
          await new sql.Request(transaction)
            .input(`nb_mid_${i}`, sql.Int, item.medicine_id)
            .input(`nb_qty_${i}`, sql.Int, receivedQty)
            .query(`
              UPDATE medicine_batches SET stock_qty = stock_qty + @nb_qty_${i}
              WHERE id = (SELECT TOP 1 id FROM medicine_batches WHERE medicine_id = @nb_mid_${i} ORDER BY id DESC)
            `);
        }
      }
    }
